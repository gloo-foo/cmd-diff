...
func emitSimple(send func([]byte) bool, input1, input2 lines) {
	for i := range longer(input1, input2) {
		emitSimpleAt(send, input1, input2, i)
	}
}

// emitSimpleAt renders one position of the ed-style diff.
func emitSimpleAt(send func([]byte) bool, input1, input2 lines, i int) {
	if bytes.Equal(input1.at(i), input2.at(i)) {
		return
	}
	if i < len(input1) {
...
func emitUnified(send func([]byte) bool, input1, input2 lines) {
	...
	for i := range longer(input1, input2) {
		emitUnifiedAt(send, input1, input2, i)
	}
}

// emitUnifiedAt renders one position of the unified-diff body.
func emitUnifiedAt(send func([]byte) bool, input1, input2 lines, i int) {
	switch {
	case i >= len(input1):
		send(fmt.Appendf(nil, "+%s", input2[i]))
	case i >= len(input2):
