...
func TestDiff_PositionalFilesViaMemFs(t *testing.T) {
	...
	lines, err := testable.TestLines(
		command.Diff(gloo.File("a.txt"), gloo.File("b.txt"), command.DiffFs{Fs: fs}),
		"ignored upstream\n",
	)
	if err != nil {
		t.Fatal(err)
...
func TestDiff_FileNotFoundPropagates(t *testing.T) {
	...
	_, err := testable.TestLines(
		command.Diff(gloo.File("missing.txt"), command.DiffFs{Fs: fs}),
		"",
	)
	if err == nil {
...
func TestDiff_Input2FileNotFoundPropagates(t *testing.T) {
	...
	_, err := testable.TestLines(
		command.Diff(gloo.File("a.txt"), gloo.File("missing.txt"), command.DiffFs{Fs: fs}),
		"",
	)
	if err == nil {
